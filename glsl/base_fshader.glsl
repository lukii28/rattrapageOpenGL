...
void main(){
   if(lighting_mode == 0u || lighting_mode == 1u){ //surface or vertex shading => take values calculated in vertex shader
      ...
      color = vec3(frag_color_out.x * frag_surface_normal_color.x, frag_color_out.y*frag_surface_normal_color.y, frag_color_out.z*frag_surface_normal_color.z);
   }
   else{
      ...
      color = vec3(frag_color_out.x * lum, frag_color_out.y*lum, frag_color_out.z*lum);
   }
